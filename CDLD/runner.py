...
class Runner(object):
    ...
    def train(self, epoch, data):
        ...
        # embeddings: static+evolve， cs: static, ss: trivial, ds: dynamic ds_new:evolve
        embeddings, cs, ss, ds, ds_new = self.model([
            data['edge_index_list'][ix].long().to(args.device)
            for ix in range(self.len)
        ], self.x)
        device = cs[0].device
        ...
        static_y = cal_y(cs, self.model.ss_decoder)
        sy = cal_y(ss, self.model.ss_decoder)
        ...
        static_loss = cal_loss(cy, edge_label)
        # dynamic loss
        dynamic_loss = 0
        for t in range(len(ds_new)-1):
            dynamic_loss+=torch.mean(torch.norm(ds_new[t]/torch.norm(ds_new[t]).reshape(-1,1)-ds[t+1]/torch.norm(ds[t]).reshape(-1,1), dim=1))
        # print(dynamic_loss)
        # de
        ...
        for i in range(intervention_times):
            s1 = np.random.randint(len(static_y))
            s = torch.sigmoid(static_y[s1]).detach()
            conf = s * cy
            # conf=self.model.comb_pred(cs,)
            env_loss = torch.cat(
                [env_loss,
        ...
        env_mean = env_loss.mean()
        env_var = torch.var(env_loss * intervention_times)
        dy_penalty = env_mean + env_var


        # loss = static_loss + la * penalty
        # loss = causal_loss 
        # print(static_loss)
        # print(dynamic_loss)
        # print(la * penalty)
        # print(la * dy_penalty)
        # loss = static_loss + dynamic_loss + la * penalty + la * dy_penalty
        loss = static_loss + la * penalty
