...
class DGNNLayer(nn.Module):
    ...
    def __init__(self, in_dim, hid_dim, n_heads, norm=True, dropout=0, skip=False, use_RTE=False, sample_r=0.1, use_fmask=False, only_causal=False):
        ...
        self.dy_q_linear = nn.Linear(in_dim, hid_dim)
        self.dy_k_linear = nn.Linear(in_dim, hid_dim)
        self.dy_v_linear = nn.Linear(in_dim, hid_dim)
        ...
        self.dy_time_emb = RelTemporalEncoding(hid_dim)
        ...
        max_t = 16
        self.timeweight = nn.Parameter(torch.randn(max_t))
    ...
    def evolve(self, ds):
        ds_new = [ds[0]*self.timeweight[0]]

        for t in range(1,len(ds)):
            ds_new.append(ds[t]*self.timeweight[t]+ds_new[t-1])
        return ds_new
    ...
    def dy_DAttn(self, x_tar, x_src, t_tar, t_src):
        """ do attention
        return attention,message
        """
        # to sparse
        ...
        if self.use_RTE:
            device = x_tar.device
            target_node_vec = self.dy_time_emb(target_node_vec, torch.LongTensor([t_tar]).to(device))
            source_node_vec = self.dy_time_emb(source_node_vec, torch.LongTensor([t_src]).to(device))

        # get weights
        q_linear, k_linear, v_linear = self.dy_q_linear, self.dy_k_linear, self.dy_v_linear
        # calculation
        q_mat = q_linear(target_node_vec).view(-1, self.n_heads, self.d_k)
        k_mat = k_linear(source_node_vec).view(-1, self.n_heads, self.d_k)
        v_mat = v_linear(source_node_vec).view(-1, self.n_heads, self.d_k)  # [E,h,F/h]
    ...
    def DAttnMulti(self, x_tar, topos):
        ...
        for (x_tar_e, x_src_e, t_tar, t_src, ei_tar) in topos:
            ...
            dy_att,_ = self.dy_DAttn(x_tar_e, x_src_e, t_tar, t_src)
            res_atts.append(att)
            res_msgs.append(msg)
            dy_res_atts.append(dy_att)
            ei_tars.append(ei_tar)
        res_att = torch.cat(res_atts, dim=0)
        res_msg = torch.cat(res_msgs, dim=0)
        dy_res_att = torch.cat(dy_res_atts, dim=0)
        ...
        dyn_att = softmax(dy_res_att, ei_tar)
        dyn = res_msg * res_att.view(-1, self.n_heads, 1) * dyn_att.view(-1, self.n_heads, 1)    # [E,h,F/h]
        dyn = dyn.view(-1, self.hid_dim)  # [E,F]

        sta_att = 1-softmax(dy_res_att, ei_tar)
        sta = res_msg * res_att.view(-1, self.n_heads, 1) * sta_att.view(-1, self.n_heads, 1)    # [E,h,F/h]
        sta = sta.view(-1, self.hid_dim)  # [E,F]
        ...
        dynamic_hat = scatter(dyn, ei_tar, dim=self.node_dim, dim_size=x_tar.shape[0], reduce=self.aggr)  # [N,F]
        static_hat = scatter(sta, ei_tar, dim=self.node_dim, dim_size=x_tar.shape[0], reduce=self.aggr)  # [N,F]
        if self.use_fmask:
        ...
        static = ffn(static_hat+x_tar)
        spurious = ffn(spurious_hat)
        dynamic = ffn(dynamic_hat+x_tar)
        ...
        return res, static, spurious, dynamic

    def vgae(self, node_embs, Ahat_norm, GCN_weights):
        node_embs = self.activation(Ahat_norm.matmul(node_embs.matmul(GCN_weights)))
        embs_mean = Ahat_norm.matmul(node_embs.matmul(GCN_weights_mean))
        embs_var = Ahat_norm.matmul(node_embs.matmul(GCN_weights_var))
        
        device = embs_var.device
        gaussian_noise = torch.randn(node_embs.shape[0], embs_var.shape[-1]).to(device)
        sampled_z = embs_mean + gaussian_noise * torch.exp(embs_var).to(device)
        
        adj_pl = torch.sigmoid(torch.matmul(sampled_z, sampled_z.t()))
        return adj_pl
    ...
    def forward(self, edge_index_list, x_list):
        ...
        for t_tar, (x_tar, topos) in enumerate(self.collect_neighbors(edge_index_list, x_list)):
            x, c, s, d = self.DAttnMulti(x_tar, topos)
            xs.append(x)
            cs.append(c)
            ss.append(s)
            ds.append(d)
        ds_new = self.evolve(ds)
        for t in range(len(ds_new)):
            xs[t] = cs[t] + ds[t]
        return xs, cs, ss, ds, ds_new
...
class DGNN(nn.Module):
    ...
    def forward(self, edge_index_list, x_list):
        ...
        for i, layer in enumerate(self.layers):
            x_list, cs, ss, ds, ds_new = layer(edge_index_list, x_list)
            if i != len(self.layers)-1:
                x_list = [self.act(x) for x in x_list]

        return x_list, cs, ss, ds, ds_new
